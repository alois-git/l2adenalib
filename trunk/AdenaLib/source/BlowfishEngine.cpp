...
int S0D[SBOX_SK] = {0xD1310BA6, 0x98DFB5AC, 0x2FFD72DB,
...
int S1D[SBOX_SK] =  {0x4B7A70E9, 0xB5B32944, 0xDB75092E,
...
int S2D[SBOX_SK]= {0xE93D5A68, 0x948140F7, 0xF64C261C,
...
int S3D[SBOX_SK]={0x3A39CE37, 0xD3FAF5CF, 0xABC27737,
...
{
	memcpy(&S0,S0D,SBOX_SK*4);
	memcpy(&S1,S1D,SBOX_SK*4);
	memcpy(&S2,S2D,SBOX_SK*4);
	memcpy(&S3,S3D,SBOX_SK*4);
	P = new int[18];
	encrypting = false;
...
/**
	...
	*/
void BlowfishEngine::init(bool encrypting, char key[])
{
	this->encrypting = encrypting;
	strncpy(this->workingKey,key,50);
	setKey(this->workingKey);
	
	return;
};
char* BlowfishEngine::getAlgorithmName()
{
	return "Blowfish";
};
int BlowfishEngine::processBlock(char in[], int inOff, char *out, int outOff)
{
	/*if (workingKey[0] == NULL)
	{
...
void BlowfishEngine::reset()
{
};
int BlowfishEngine::getBlockSize()
{
	return BLOCK_SIZE;
};
//==================================
...
//==================================
int BlowfishEngine::F(int x)
{
	return (((S0[((unsigned)x >> 24)] + S1[((unsigned)x >> 16) & 0xff]) ^ S2[((unsigned)x >> 8) & 0xff]) + S3[(unsigned)x & 0xff]);
};
/**
	* apply the encryption cycle to each value pair in the table.
	*/
void BlowfishEngine::processTable(int xl, int xr, int *table, int size )
{
...
void BlowfishEngine::setKey(char key[])
{
	/*
		...
		*/
	/*
	arraycopy(KS0, 0, S0, 0, SBOX_SK);
	arraycopy(KS1, 0, S1, 0, SBOX_SK);
	arraycopy(KS2, 0, S2, 0, SBOX_SK);
	arraycopy(KS3, 0, S3, 0, SBOX_SK);
	arraycopy(KP, 0, P, 0, P_SZ);*/
	...
	/*int null1 = 0;
	int null2 = 0;*/
	processTable(0, 0, (int*)P,18);
	processTable(P[P_SZ - 2], P[P_SZ - 1], (int*)S0, 256);
	processTable(S0[SBOX_SK - 2], S0[SBOX_SK - 1], (int*)S1,256);
	processTable(S1[SBOX_SK - 2], S1[SBOX_SK - 1], (int*)S2,256);
	processTable(S2[SBOX_SK - 2], S2[SBOX_SK - 1], (int*)S3,256);
};
...
/**
	...
	*/
void BlowfishEngine::encryptBlock(char src[], int srcIndex, char *dst, int dstIndex)
{
	...
	xr = xr ^ P[ROUNDS + 1];
	dst[dstIndex] = (char)xr;
	dst[dstIndex +1] = (char)(xr >> 8);
	dst[dstIndex +2] = (char)(xr >> 16);
	dst[dstIndex + 3] = (char)(xr >> 24);
	dst[dstIndex + 4] = (char)xl;
	dst[dstIndex + 1 + 4] = (char)(xl >> 8);
	dst[dstIndex + 2 + 4] = (char)(xl >> 16);
	dst[dstIndex + 3 + 4] = (char)(xl >> 24);
...
/**
	...
	* will be an exact multiple of our blocksize.
	*/
void BlowfishEngine::decryptBlock(char src[], int srcIndex, char *dst, int dstIndex)
{
	int xl = BytesTo32bits(src, srcIndex);
	int xr = BytesTo32bits(src, srcIndex + 4);
	xl = xl ^ P[ROUNDS + 1];
...
int BlowfishEngine::BytesTo32bits(char b[], int i) //just enfin a l'air : 2fois !
{
...
void BlowfishEngine::Bits32ToBytes(int in, char *b[], int offset)
{
	*b[offset] = (char)in;
